...
public class BitwiseOps {
    ...
    /**
     * Returns the first k bits of the given long (first from the left, ie. higher order).
     *
    ...
    public static long getFirstKBits(long x, int k) {
        return x >>> (BaseTransTable.HASH_SIZE - k);
    }

    /**
     * Returns the last k bits of the given long (last from the left, ie. lower order).
     *
     * @param x the long to extract the bits from
     * @param k the number of bits to extract
